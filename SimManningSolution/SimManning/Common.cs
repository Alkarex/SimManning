...
namespace SimManning
{
	public static class SimManningCommon
	{
		...
		/// Special Cases:
		/// • If the argument is NaN, the result is Double.NaN.
		/// • If the argument is positive infinity, the result is Double.PositiveInfinity.
		/// • If the argument is zero, the result is Double.Epsilon
		/// </summary>
		...
		/// <remarks>
		/// Like in Java http://java.sun.com/javase/6/docs/api/java/lang/Math.html#nextUp(double)
		/// </remarks>
		public static double NextUp(this double d)
		{
			if (Double.IsNaN(d) || Double.IsPositiveInfinity(d)) return d;
			d += 0.0;
			return BitConverter.Int64BitsToDouble(BitConverter.DoubleToInt64Bits(d) + ((d >= 0.0) ? 1L : -1L));
		}

		public static double NextDown(this double d)
		{
			if (Double.IsNaN(d) || Double.IsNegativeInfinity(d)) return d;
			d -= 0.0;
			return BitConverter.Int64BitsToDouble(BitConverter.DoubleToInt64Bits(d) + ((d >= 0.0) ? -1L : 1L));
		}*/
		...
		/// <returns><code>true</code> if <para>code</para> is a sub-code of <para>code2</para> (ex: 357 is a subcode of 35), <code>false</code> otherwise (ex: 357 is not a subcode of 400, or of 3571)</returns>
		public static bool IsSubCodeOf(this int code, SimulationTask.StandardType code2)
		{
